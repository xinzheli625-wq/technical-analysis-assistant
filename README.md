...
在 `.env` 中配置，所有自动生成的文档都会落到该文件夹：

```bash
# 从飞书文件夹 URL 提取：https://my.feishu.cn/drive/folder/<folder_token>
FEISHU_FOLDER_TOKEN=
```

写入个人文件夹需要授权（二选一）：

1. **把文件夹共享给应用（bot 身份，免登录）**：在飞书文件夹的共享设置中把 lark-cli 对应的应用加为协作者；
2. **用户身份调用**：`lark-cli auth login --domain docs,drive` 完成设备授权，然后在 `.env` 设置 `FEISHU_AS=user`。
...
| Token 用量 | `data/token_usage.jsonl` | 每次 LLM 调用的 prompt/completion/缓存命中明细 |

### LLM 成本（DeepSeek V4 Pro 实测）

一次完整分析（Step 1-8）只有 Step 5 的 `analyze_full` 一次 LLM 调用产生费用。
2026-07 实测单次：prompt **21.8k tokens** + completion **5.3k tokens** ≈ **27k tokens**。

| 场景 | 单次成本 | 说明 |
|------|---------|------|
| 缓存热命中（同日连续分析/跟踪，常态） | **≈ ¥0.03** | 静态 prompt（Skill 索引等约 21.7k tokens）99%+ 命中上下文缓存 |
| 冷启动（当天首次） | ≈ ¥0.10 | 输入全按未命中计费 |
| 高峰时段（9-12 / 14-18 点） | 热 ¥0.06 / 冷 ¥0.19 | 官方峰谷定价 ×2 |

定价依据（2026-07 官方永久价）：输入（缓存未命中）¥3 / 百万 tokens、
输入（缓存命中）¥0.025 / 百万、输出 ¥6 / 百万。
**量级结论：每天分析 10 只股票，一个月 LLM 成本约 ¥10-30。**
成本大头是静态 prompt，DeepSeek 上下文缓存把它压到接近免费；
JSON 解析修复全部在本地完成，解析失败不产生额外 API 调用。
...
| `FEISHU_FOLDER_TOKEN` | 否 | 飞书文档目标文件夹（从文件夹 URL 提取），不配置则自动创建"技术分析助手"文件夹 |
| `FEISHU_AS` | 否 | 飞书调用身份 `bot`（默认）/ `user`（个人文件夹写入需先 `lark-cli auth login`） |
